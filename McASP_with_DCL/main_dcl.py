...
from mcasp_model_ import McASP
import datetime
...
def rank_by_length(train_path, dataset_name):
    '''
    对训练数据根据长度进行排序
    :param train_path:
    :param dataset_name:
    ...
    for i in range(len(text)):
        if text[i] !='' and text[i] !='\n':
            t = text[i].split('\n')
            id_length[i] = len(t)
    index_sort = sorted(id_length.items(), key=lambda x: x[1])
    index = [item[0] for item in index_sort]
    # print(len(index))
    f1 =open('./sample_data/' + dataset_name + '/train_ranklength.txt','w',encoding='utf-8')
    for x in index:
        f1.write(str(x)+'\n')
...
def train(args):
    ...
    if args.use_attention:
        ...
        ngram_files = [args.train_data_path, args.eval_data_path]
    ...
    if args.dataset_name == 'zz':
        label_list = ['PAD', 'S_w', 'S_ns', 'S_d', 'S_v', 'S_n', 'S_p', 'B_ns', 'E_ns', 'S_nr', 'S_u', 'S_y', 'S_r',
                      'S_a', 'B_nr', 'E_nr', 'S_c', 'B_n', 'E_n', 'B_v', 'E_v', 'S_m', 'S_j', 'M_nr', 'S_f', 'B_t',
                      'E_t', 'M_n', 'B_a', 'E_a', 'B_d', 'E_d', 'B_r', 'E_r', 'M_t', 'B_m', 'E_m', 'S_q', 'B_c', 'E_c',
                      'S_sv', 'S_t', 'B_p', 'E_p', 'M_ns', 'B_y', 'E_y', 'M_m', 'B_w', 'E_w', 'M_r', 'M_v', 'B_nsr',
                      'E_nsr', 'S_wv', 'B_f', 'E_f', 'S_b', 'M_c', 'M_a', 'B_rs', 'E_rs', 'S_mr', 'B_s', 'E_s', 'B_rr',
                      'M_rr', 'E_rr', 'S_s', 'S_yv', 'B_nn', 'E_nn', 'B_mr', 'E_mr', 'B_rn', 'E_rn', 'B_u', 'E_u',
                      'S_rs', 'M_y', 'M_f']
        label_list += ['[CLS]', '[SEP]']
        ...
        if args.dataset_name == 'sg_gerneral':
            pos = ['A', 'ADV', 'DC', 'DF', 'DJ', 'ASP', 'C', 'M', 'FW', 'NA', 'NA4', 'NA5', 'NI', 'NB1', 'NB2', 'NB3',
                   'NB4', 'NB5', 'NH', 'NG', 'P', 'T', 'I', 'VI', 'VT', 'VP', 'U']
    ...
    tagger = McASP(word2id, gram2id, gram2count, label_map, hpara, args, model_path=args.bert_model)

    train_examples = tagger.load_tsv_data(args.train_data_path)

    eval_examples = tagger.load_tsv_data(args.eval_data_path)
    ...
    nb_tr_steps = 0
    tr_loss = 0
    # train_features = convert_examples_to_features(batch_examples, train_index, args,
    #                                               'train', )  # 按照一个batch处理，就要记住这个batch 之内是哪些对应的数据,根据start_index 和len(batch_examples)来确定
    ...
    if args.do_train:
        ...
        if args.use_data_level:
            ...
            #     difficult_mode = 'bayesian'
                ...
                difficult_mode = 'length'
                # baseline 先对训练集的数据进行长度排序
                rank_by_length(args.train_data_path, args.dataset_name)
                print('For baseline! Use length of sentence as the difficulty metric.')
            rank_index = []

            for line in open('./sample_data/' + args.dataset_name + '/train_rank' + difficult_mode + '.txt'):
                if line != '\n':
                    rank_index.append(int(line.strip()))
            # print(len(rank_index), len(train_examples))
            # exit()
            for x in rank_index:  # 将训练集按照原来data-level 的难度排列，train_samples_rank.txt 中记录的是每个sample的index
                #rank_index 中是所有数据排序，ID
                id2train[x] = train_examples[x]
        ...
        '''在训练之前一次性读取所有数据'''
        #train_examples始终是原来的顺序
        train_rank_index = [i for i in range(len(train_examples))]
        train_features = convert_examples_to_features(train_examples)  # 一次性处理所有的训练数据，减少训练过程冲多次读数据的风险
        # print(len(train_features))
        # exit()
        index = [i for i in range(len(eval_examples))]
        eval_features = convert_examples_to_features(eval_examples)

        id2feature = {}
        for i in rank_index:
            id2feature[i] = train_features[i]  # key是数据的在原始数据集中的id,value是生成的相应的训练数据，之后每次按照id取训练数据即可
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            if args.use_curri and last_f == 0:
                if rank_index != last_rank_index:
                    ...
                    print('上一次训练数据index：', len(last_train_examples))
            ...
            for step, start_index in enumerate(tqdm(range(0, len(train_examples), args.train_batch_size))):
                ...
                train_features_batch = [id2feature[i] for i in batch_index]
            ...
            segment_ids, valid_ids, word_ids, word_mask = feature2input(
                    device, train_features_batch)

                max_loss, loss, seq, _  = tagger(input_ids, segment_ids, input_mask, label_ids, valid_ids, l_mask,
            ...
            if args.local_rank == -1 or torch.distributed.get_rank() == 0:
                ...
                for start_index in range(0, len(eval_examples), args.eval_batch_size):
                    ...
                    batch_index = [i for i in range(start_index, start_index + len(eval_batch_examples))]

                    eval_features_batch  = [eval_features[i] for i in batch_index]

                    # eval_features_batch = convert_examples_to_features(eval_batch_examples, index, args, 'test')

                    channel_ids, input_ids, input_mask, l_mask, label_ids, matching_matrix, ngram_ids, ngram_positions, \
                    segment_ids, valid_ids, word_ids, word_mask = feature2input(
                        device, eval_features_batch)

                    with torch.no_grad():
                       max_lossi, total_loss, tag_seq, logits = tagger(input_ids, segment_ids, input_mask, labels=label_ids,
                        ...
                        # tagger(input_ids, segment_ids, input_mask, label_ids, label_cls_id, valid_ids, l_mask, word_ids,
                ...
                oov = cws_evaluate_OOV(y_pred, y_true, sentence_all,
                                       word2id)  # 这里其实穿的word2id 参数不对，因为，这里是训练集和测试集所有词的都在，在这里评估的也是测试集
                ...
                if pf > best_f:
                    ...
                    if args.dataset_name:
                        ...
                        if n_gpu > 1:
                            torch.save({
                                'spec': tagger.module.spec,
                                'state_dict': tagger.module.state_dict(),
                    ...
                    if args.use_curri:
                        ...
                        for start_index in range(0, len(rank_examples), args.eval_batch_size):
                            eval_batch_examples = rank_examples[start_index: min(start_index + args.eval_batch_size,
                                                                                 len(rank_examples))]
                            init_index = init_rank_index[start_index: (start_index+len(eval_batch_examples))]

                            # rank_features = convert_examples_to_features(eval_batch_examples, init_index, args, 'train')
                            rank_features = [train_features[i] for i in init_index ]

                            channel_ids, input_ids, input_mask, l_mask, label_ids, matching_matrix, ngram_ids, ngram_positions, \
                    segment_ids, valid_ids, word_ids, word_mask = feature2input(
                                device, rank_features)
                            with torch.no_grad():
                                if args.use_bayesian:
                                    difficult_mode = 'bayesian'
                                    ...
                                    for i in range(args.dropout_times):
                                        '''测试多次，只在排序时使用'''
                                        maxloss, total_loss, _, prob = tagger(input_ids, segment_ids, input_mask, labels=label_ids,
                                         valid_ids=valid_ids, attention_mask_label=l_mask,
                                         word_seq=word_ids, label_value_matrix=matching_matrix,
                                         word_mask=word_mask, channel_ids=channel_ids,
                                         input_ngram_ids=ngram_ids, ngram_position_matrix=ngram_positions, rank=False)
                                    ...
                                    difficult_mode = 'LC'
                                    maxloss, total_loss, _, logits = tagger(input_ids, segment_ids, input_mask, labels=label_ids,
                                         valid_ids=valid_ids, attention_mask_label=l_mask,
                                         word_seq=word_ids, label_value_matrix=matching_matrix,
                                         word_mask=word_mask, channel_ids=channel_ids,
                                         input_ngram_ids=ngram_ids, ngram_position_matrix=ngram_positions)
                                    ...
                                    # exit()
                                    n = args.top_n #原来在在这里设置的是top-n ,即计算句子中的最高的几个不确定性的token
                                    ...
                                    # print("Doesn't use any curriculum rank metric, only retain the original data rank.")
                                    #baseline 也是不在模型训练过程中对数据进行排序，依然使用原始的长度排序
                                    maxloss, total_loss, _, logits = tagger(input_ids, segment_ids, input_mask, labels=label_ids,
                                         valid_ids=valid_ids, attention_mask_label=l_mask,
                                         word_seq=word_ids, label_value_matrix=matching_matrix,
                                         word_mask=word_mask, channel_ids=channel_ids,
                        ...
                        index2var = sorted(index2var.items(), key=lambda x: x[
                            1])  # 按照var值进行排序 [(2453, 0.01), (1312, 0.02), (1341, 0.04), (1351, 0.12)]
...
def rank_sample(args):
    ...
    tagger_checkpoint = torch.load(args.eval_model)
    # print(tagger_checkpoint['state_dict'].keys())
    #     exit()
    tagger = McASP.from_spec(tagger_checkpoint['spec'], tagger_checkpoint['state_dict'], args)
    # label_map = tagger.label_map

    convert_examples_to_features = tagger.convert_examples_to_features
    feature2input = tagger.feature2input
    num_labels = tagger.num_labels
    word2id = tagger.word2id
    voc2id = tagger.voc2id
    id2labels = {}
    label_map = {v: k for k, v in tagger.labelmap.items()}
    label_list = []
    ...
    if args.fp16:
        tagger.half()
    eval_examples = tagger.load_data(args.train_data_path, label_list, args)  # 对训练数据排序
            ...
            if args.use_bayesian: 
                ...
                for i in range(args.dropout_times):
                    ...
                    maxloss, total_loss, _, prob = tagger(input_ids, segment_ids, input_mask, labels=label_ids,
                                         ...
                                         word_seq=word_ids, label_value_matrix=matching_matrix,
                                         word_mask=word_mask, channel_ids=channel_ids,
                                         input_ngram_ids=ngram_ids, ngram_position_matrix=ngram_positions, rank=True)
                    max_loss.append(maxloss)
                    sample_loss.append(total_loss)
    ...
    index2var = sorted(index2var.items(),
                       key=lambda x: x[1])  # 按照var值进行排序 [(2453, 0.01), (1312, 0.02), (1341, 0.04), (1351, 0.12)]
    rank_index = [t[0] for t in index2var]  # 得到难度排序的index,从易到难，var越小，越简单
...
# def test(args):
    ...
    # # print(tagger_checkpoint['state_dict'].keys())
    # #     exit()
    # tagger = WMSeg.from_spec(tagger_checkpoint['spec'], tagger_checkpoint['state_dict'], args)
    # # label_map = tagger.label_map
    ...
    # word2id = tagger.word2id
    # voc2id = tagger.voc2id
    # # dataset_map = tagger.dataset_map
    ...
    # # big_word2id = tagger.big_word2id
    # # dicts2id = tagger.dict2id
    # # dataset_map = tagger.dataset_map
    ...
    # if args.fp16:
    #     tagger.half()
    # eval_examples = tagger.load_data(args.test_data_path, label_list, args)
    # tagger.to(device)
    ...
    #     eval_features_batch = [eval_features[i] for i in index]

    #     # eval_features = convert_examples_to_features(eval_batch_examples, index, args, 'test')

    #     input_ids, input_mask, l_mask, label_ids, segment_ids, valid_ids, fw, bw, syn, srl, wg, c_ids, syn_ids, srl_ids = feature2input(
    #         device, eval_features_batch )

    #     with torch.no_grad():
    #         _, _, tag_seq, _ = tagger(input_ids=input_ids, token_type_ids=segment_ids,
...
def test(args):
    ...
    tagger = McASP.load_model(args.eval_model, args)
    ...
    for start_index in tqdm(range(0, len(eval_examples), args.eval_batch_size)):
        ...
        with torch.no_grad():
            _, _, tag_seq, _  = tagger(input_ids, segment_ids, input_mask, labels=label_ids,
                             valid_ids=valid_ids, attention_mask_label=l_mask,
                             word_seq=word_ids, label_value_matrix=matching_matrix,
                             word_mask=word_mask, channel_ids=channel_ids,
                             input_ngram_ids=ngram_ids, ngram_position_matrix=ngram_positions)
    ...
    for example, y_true_item in zip(eval_examples, y_true):
        ...
        sen = sen.strip()
        sen = sen.split(' ')
        # sen = [x.replace('M','I') for x in sen]
        # y_true_item = [x.replace('M','I') for x in y_true_item]
        if len(y_true_item) != len(sen):
            print(len(sen))
            sen = sen[:len(y_true_item)]
        sentence_all.append(sen)

    y_true_all = [x.replace('M','I') for x in y_true_all]
    y_pred_all = [x.replace('M','I') for x in y_pred_all]
    (wp, wr, wf), (pp, pr, pf) = pos_evaluate_word_PRF(y_pred_all, y_true_all)
    woov = cws_evaluate_OOV(y_pred, y_true, sentence_all, word2id)
    
    output_model_dir = args.eval_model.split('/model')[1] + '/'
    print("\nCWS_P: %f, CWS_R: %f, CWS_F: %f, CWS_OOV: %f" % (wp, wr, wf, woov))
    print("\nPOS_P: %f, POS_R: %f, POS_F: %f" % (pp, pr, pf))
    f = open(output_model_dir + 'test_metrics.tsv', 'w', encoding='utf8')

        # print(args.test_data_path)
    f.write("\nCWS_P: %f, CWS_R: %f, CWS_F: %f, CWS_OOV: %f\n" % (wp, wr, wf, woov))
    f.write("\nPOS_P: %f, POS_R: %f, POS_F: %f" % (pp, pr, pf))
    # print("\nCWS_P: %f, CWS_R: %f, CWS_F: %f, CWS_OOV: %f" % (wp, wr, wf, woov))
    # print("\nPOS_P: %f, POS_R: %f, POS_F: %f" % (pp, pr, pf))
...
def main():
    ...
    parser.add_argument("--voc", type=str, default='voc_train_dev_ngrams_jieba.txt',
                        help="vocabulary type:voc_train_test.txt or voc_train_jieba.txt")
    ...
    parser.add_argument("--top_n", type=int, default=5,
                        help="The length top-n in use_top_k_LC curriculum")
    parser.add_argument("--use_nor_log_P", action='store_true',
                        help="Whether to use normalize log probability as difficulty")
    parser.add_argument("--use_length", action='store_true',
                        help="Whether to use length (baseline) probability as difficulty")
