...
def train(args):
    ...
    if args.do_train:
        ...
        train_features = convert_examples_to_features(train_examples, train_rank_index , args,
                                                      'train', )  # 一次性处理所有的训练数据，减少训练过程冲多次读数据的风险
        # print(len(train_features))
        # exit()
        index = [i for i in range(len(eval_examples))]
        eval_features = convert_examples_to_features(eval_examples, index, args, 'test')
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            if args.local_rank == -1 or torch.distributed.get_rank() == 0:
                ...
                for start_index in range(0, len(eval_examples), args.eval_batch_size):
                    ...
                    with torch.no_grad():
                        max_lossi1, total_loss1, tag_seq, logits = seg_model(input_ids=input_ids,
...
def rank_sample(args):
    ...
    index = [i for i in range(len(eval_examples))]
    eval_features = convert_examples_to_features(eval_examples, index, args, 'train')
    seg_model.to(device)
