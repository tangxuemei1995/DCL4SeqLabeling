...
#         exit()
        ...
        if self.istraining or rank:#测试时也使用dropout
            # print('dropout')
            # if not self.eval: #手动屏蔽
            self.dropout_MC = nn.Dropout(0.5) #不能写在_init_ 否则eval时不能生效
#
            sequence_output = self.dropout_MC(sequence_output)
        # print(sequence_output.size())
 #        exit()
## print(sequence_output[0][0][0].value)
...
#             # Only keep active parts of the loss
        # else:
        max_lossi1, total_loss1, tag_seq , logits= self.softmax(sequence_output, attention_mask, labels)
...
#maxloss_aver  = torch.div( maxloss_, 4)
        # print(maxloss_aver)
#         exit()
        return max_lossi1, total_loss1, tag_seq, logits
...
#                         print(label)
 ...
 #                    poses.append(pos)
    ...
    def convert_examples_to_features(self, examples, index, args, mode='train' ):
        ...
        if args.use_gcn or args.use_attention: #使用gcn和词表attention
            ...
            for i in range(len(index)):
                ...
                x = index[i]
                # print(x)
                graph_file_syn = os.path.join('sample_data', args.dataset_name, 'graph', mode, 'syn', str(x) + '.npy')
                graph_file_srl = os.path.join('sample_data', args.dataset_name, 'graph', mode, 'srl', str(x) + '.npy')
                
                # print(graph_file)
#                 exit()
                if not os.path.isfile(graph_file_syn):
            ...
            forward= [csr_matrix(np.zeros([graph_max_length_dep, graph_max_length_dep]))] * len(examples)
            backward = [csr_matrix(np.zeros([graph_max_length_dep, graph_max_length_dep]))] * len(examples)
            word_attention = [csr_matrix(np.zeros([graph_max_length_word, graph_max_length_word]))] * len(examples)
            syn_graphs = [csr_matrix(np.zeros([graph_max_length_syn, graph_max_length_syn]))] * len(examples)
            candidate_id = [[0] * (graph_max_length_word -args.max_seq_length)] * len(examples)
            syn_word_ids = [[0] * (graph_max_length_syn - args.max_seq_length)] * len(examples)
            srl_graphs =  [csr_matrix(np.zeros([args.max_seq_length + 24, args.max_seq_length + 24]))] * len(examples)
            srl_word_ids = [[0]*24]*len(examples)
            
             # forward = np.random.randint(0,1,size=[ args.max_seq_length, args.max_seq_length, len(examples)])
#              backward = np.random.randint(0,1,size=[ args.max_seq_length, args.max_seq_length, len(examples)])
#              word_attention = np.random.randint(0,1,size=[ args.max_seq_length, args.max_seq_length, len(examples)])
#              syn_graphs =  np.random.randint(0,1,size=[ args.max_seq_length, args.max_seq_length, len(examples)])
#              candidate_id = np.random.randint(0,1,size=[ args.max_seq_length, len(examples)])
#              syn_word_ids = np.random.randint(0,1,size=[ args.max_seq_length, len(examples)])
#              srl_graphs =  np.random.randint(0,1,size=[ args.max_seq_length, args.max_seq_length, len(examples)])
#              srl_word_ids = np.random.randint(0,1,size=[ args.max_seq_length, len(examples)])
