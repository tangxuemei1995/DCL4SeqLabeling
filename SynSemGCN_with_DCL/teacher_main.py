...
#         num_train_optimization_steps = num_train_optimization_steps // torch.distributed.get_world_size()
    ...
    nb_tr_steps = 0
    tr_loss = 0
    new_train_exapmles = teacher_model(args, train_examples)
    '''得到来自教室模型排序之后的训练集'''
    if args.do_train:
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            '''先对训练集的数据进行筛选，使用线性增加的方式'''
            # if epoch == 2:
#                 exit()
            initial_P = 0.1 #初始使用20%的数据
            if epoch < args.num_train_epochs-5:
                a = min(1, initial_P + ((1-initial_P) / (args.num_train_epochs-5)) * epoch)
                train_examples = new_train_exapmles[0:int(len(new_train_exapmles)*a)]
                # print
            else:
                train_examples = new_train_exapmles
...
#                 exit()
                ...
                fr.write('a:\t' + str(a))
                fr.write('train_examples:\t' + str(len(train_examples)))
                for key in dataset_map.keys():
...
def teacher_model(args,train_examples):
        print('use teacher model to sort training set!')
     ...
     #    optimizer.load_state_dict(checkpoint['optimizer'])
        ...
        seg_model_checkpoint = torch.load(args.teacher_model)
    ...
    #     exit()
        ...
        y_true, y_cls_true= [], []
        y_pred, y_cls_pre = [], []
        train_examples = {} #key=loss, value = example
        for start_index in tqdm(range(0, len(eval_examples), args.eval_batch_size)):
            ...
            '''按照损失对训练集数据进行记录'''
            for key, t in zip(eval_batch_examples, sort_loss):
                if train_examples.get(key) is None:
                    train_examples[key] = t
                else:
                    train_examples[key] = t
        new_train_examples = sorted(train_examples.items(), key = lambda x:x[1])
        output_model_dir = os.path.join('./output', args.dataset_name + '_' + args.model_set)
        # print()
        train_examples = []
        
        f = open(os.path.join(output_model_dir, 'train_sorted.tsv'),'w', encoding='utf8')
        for x in new_train_examples:
            train_examples.append(x[0])
            f.write(''.join(x[0].text_a) + '\n')
        # exit()
...
def main():
    ...
    parser.add_argument("--teacher_model", default=None, type=str,
                        help="this model used for sort training data according to loss")
